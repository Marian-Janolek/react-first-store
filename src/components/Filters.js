...
const Filters = () => {
  const {filters: {
    text, category, color, min_price, price, max_price, shipping, company
            ...
            <div className="colors">
              {
                colors.map((c, index)=>{
                  if(c === 'all'){
                    return <button name='color' onClick={updateFilters} key={index} data-color='all' className={`${color === 'all'? 'all-btn active' : 'all-btn'}`}>all</button>
                  }
